...
public class HVERequestpayload {

	private static Logger log = Logger.getLogger(HVERequestpayload.class);
	...
	/**
	 * Get the body to post to receive HVE API payload request
	 * 
	 * 
	 * @param String
	...
	@Given("^HVE Read input from json \"([^\"]*)\" with \"([^\"]*)\" and \"([^\"]*)\" as header$")
	public RequestSpecification forhveapirequest(String filename, String username, String proxymsgid)
			throws IOException {
	 ...
	 * @throws Exception
	 */
	@When("^HVE Request is performing \"([^\"]*)\" on HVE API URL$")
	public Response post_request_HVEAPIService(String Op) throws Exception {
		log.info("*********** in WHEN  ---   posting the request to HVE API URL  *****************  ");
		String Resourcepath = props.getProperty("HVEAPIResourcePath");
		this.operation = Op;
	...
	/**
	 * Save the response as HVE API pauload service request file
	 * 
	 * 
	 * @param String
	 *            filename
	 ...
	 * @throws Exception
	 */
	@Then("^HVE Save response code in xml \"([^\"]*)\"$")
	public void saveHveResponse(String filename) throws Exception {
		...
		if (response.getStatusCode() == 200) {
			...
			Assert.assertEquals("Request cointain ns2:AlternativeLoanIdentifications/", true,
					response.then().extract().asString().contains("<ns2:AlternativeLoanIdentifications/>"));
			log.info("Request cointain ns2:AlternativeLoanIdentifications/");
			scenario.write("Request cointain ns2:AlternativeLoanIdentifications/");
		} else {
			log.info("response code received is :" + response.getStatusCode());
			scenario.write("response code received is :" + response.getStatusCode());
	...
	/**
	 * Validate API HVE request Payload file elements as per xpath
	 * 
	 * 
	 * @param String
	...
	@Then("^HVE Verify the value for below xpath elements in \"([^\"]*)\"$")
